...
public class Disk_Details extends javax.swing.JPanel {
    ...
    public void ProcessHandler(){
        ...
        if (inbyte2 == 0)
        {
            try{
                ...
                System.out.println("update useage table by dpf");
    ...
    private class ChangeLabel implements Runnable{
        ...
        ChangeLabel(int diskid, int freesize, int freeoffset, byte[] usage){
            ...
            System.out.println("dpf specail change label");
            int usen=freeoffset*100/(freesize+freeoffset);
            int i;
            for(i=0;i<usen;i++)usage[i]=1;
        }
        public void run() {
            System.out.println("change usage label");
